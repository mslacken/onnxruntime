...
TEST_P(ModelTest, Run) {
  ...
  std::unique_ptr<TestModelInfo> model_info(TestModelInfo::LoadOnnxModel(model_path.c_str()));
  ...
  for (bool is_single_thread : use_single_thread) {
    for (ExecutionMode execution_mode : execution_modes) {
      ...
      if (st.Code() == NOT_IMPLEMENTED) return;
...
INSTANTIATE_TEST_SUITE_P(ModelTests,
                         ModelTest,
                         testing::ValuesIn(GetParameterStrings()));
